...
export function SettingsContainer() {
  ...
  const handleNotificationSave = (e: React.FormEvent) => {
    e.preventDefault()
    toast.success(`Notifications saved! Daily emails scheduled at ${notifyTime}.`)
  }
          ...
          <CardContent>
            <form onSubmit={handleProfileSave} className="space-y-4">
              <div className="flex items-center gap-4 border-b border-border pb-4 mb-4">
                <Avatar className="h-12 w-12 bg-brand/20 border border-brand/30">
                  ...
                  <AvatarFallback className="text-base font-bold text-brand font-mono">
                    {user?.name ? user.name.slice(0, 2).toUpperCase() : <UserIcon className="h-5 w-5" />}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <h4 className="text-xs font-semibold text-text-primary">{user?.name || name}</h4>
                  <p className="text-[10px] text-text-muted mt-0.5">{user?.role || "USER"} &middot; {user?.email}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <Button type="submit" className="bg-brand text-text-inverse hover:bg-brand-hover text-xs font-semibold h-10 px-5 rounded-lg">
                  Save changes
                </Button>
                <Button
                  type="button"
