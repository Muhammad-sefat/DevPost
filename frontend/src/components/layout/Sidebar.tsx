...
export function Sidebar() {
  ...
  const navItems = [
    ...
    { label: "Post History", href: "/history", icon: Clock },
  ...
  return (
    <>
      ...
      <nav className="md:hidden flex fixed bottom-0 left-0 right-0 h-16 bg-bg-surface border-t border-border justify-around items-center px-4 z-30">
        {navItems.map((item) => {
          const Icon = item.icon
          const isActive = pathname === item.href
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex flex-col items-center gap-1 py-1 px-3 rounded-md transition-all duration-200",
                isActive
                  ? "text-brand"
